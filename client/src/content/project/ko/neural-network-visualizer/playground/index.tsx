...
export default function NeuralNetworkVisualizer({ onChange }: NeuralNetworkVisualizerProps) {
  ...
  const trainModel = async () => {
    if (!model || dataset.length < 10) {
      alert("학습을 시작하기 전에 적어도 10개의 데이터 포인트를 추가해주세요.");
      return;
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card className="p-4 overflow-hidden">
        <h3 className="text-lg font-bold mb-4">신경망 아키텍처</h3>
        <div className="flex flex-col space-y-4 mb-4">
          <div className="flex items-center space-x-4">
            <Label htmlFor="learning-rate">학습률: {learningRate}</Label>
            <Slider 
              id="learning-rate"
              min={0.001}
              max={0.1}
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="epochs">에폭: {epochs}</Label>
            <Slider 
              id="epochs"
              min={10}
              max={500}
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="show-activations">활성화 표시</Label>
            <Switch 
              id="show-activations" 
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="show-weights">가중치 표시</Label>
            <Switch 
              id="show-weights" 
              checked={showWeights} 
      ...
      <Card className="p-4">
        <h3 className="text-lg font-bold mb-4">학습 데이터 및 시각화</h3>
        <Tabs defaultValue="data">
          <TabsList className="mb-4">
            <TabsTrigger value="data">데이터</TabsTrigger>
            <TabsTrigger value="loss">손실</TabsTrigger>
          </TabsList>
          
          <TabsContent value="data" className="space-y-4">
            <div className="text-center mb-2">
              <p className="text-sm text-muted-foreground">
                클릭하여 포인트 추가 (왼쪽: A 클래스, 오른쪽: B 클래스)
              </p>
            </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {isTraining ? `학습 중 (${currentEpoch}/${epochs})` : '모델 학습'}
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="loss">
            ...
            {loss.length > 0 && (
              <div className="text-center">
                <p className="text-sm">최종 손실: {loss[loss.length - 1].toFixed(4)}</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
