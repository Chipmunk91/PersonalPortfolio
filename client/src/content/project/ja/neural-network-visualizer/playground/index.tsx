...
export default function NeuralNetworkVisualizer({ onChange }: NeuralNetworkVisualizerProps) {
  ...
  const trainModel = async () => {
    if (!model || dataset.length < 10) {
      alert("データポイントを少なくとも10個追加してから学習を開始してください。");
      return;
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card className="p-4 overflow-hidden">
        <h3 className="text-lg font-bold mb-4">ニューラルネットワークアーキテクチャ</h3>
        <div className="flex flex-col space-y-4 mb-4">
          <div className="flex items-center space-x-4">
            <Label htmlFor="learning-rate">学習率: {learningRate}</Label>
            <Slider 
              id="learning-rate"
              min={0.001}
              max={0.1}
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="epochs">エポック数: {epochs}</Label>
            <Slider 
              id="epochs"
              min={10}
              max={500}
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="show-activations">活性化表示</Label>
            <Switch 
              id="show-activations" 
          ...
          <div className="flex items-center space-x-4">
            <Label htmlFor="show-weights">重み表示</Label>
            <Switch 
              id="show-weights" 
              checked={showWeights} 
      ...
      <Card className="p-4">
        <h3 className="text-lg font-bold mb-4">学習データと可視化</h3>
        <Tabs defaultValue="data">
          <TabsList className="mb-4">
            <TabsTrigger value="data">データ</TabsTrigger>
            <TabsTrigger value="loss">損失</TabsTrigger>
          </TabsList>
          
          <TabsContent value="data" className="space-y-4">
            <div className="text-center mb-2">
              <p className="text-sm text-muted-foreground">
                クリックしてポイントを追加（左：クラスA、右：クラスB）
              </p>
            </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {isTraining ? `学習中 (${currentEpoch}/${epochs})` : 'モデル学習'}
              </Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="loss">
            ...
            {loss.length > 0 && (
              <div className="text-center">
                <p className="text-sm">最終損失: {loss[loss.length - 1].toFixed(4)}</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
