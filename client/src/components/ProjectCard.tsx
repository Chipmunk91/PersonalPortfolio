          ...
          <div className="flex flex-wrap gap-4 items-center justify-between mt-auto">
            ...
            <div className="flex gap-4">
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  // Load the paper content based on the current language
                  if (project.dirName) {
                    const content = getProjectPaper(project.dirName, language);
      ...
      <Dialog open={showTheory} onOpenChange={setShowTheory}>
        <DialogContent className="w-full sm:w-[80vw] sm:max-w-[80%] max-h-[80vh] overflow-y-auto" style={{ maxWidth: "80%" }}>
          ...
          <div className="prose dark:prose-invert max-w-none mt-4">
            {paperContent ? (
              <MarkdownRenderer markdown={paperContent} />
            ) : (
              <>
      ...
      <Dialog open={showVideoModal} onOpenChange={setShowVideoModal}>
        <DialogContent className="max-w-4xl">
          ...
          <div className="aspect-video bg-black rounded-md overflow-hidden">
            {project.dirName && (() => {
              // Load the language-specific video data
              const videoData = getProjectVideo(project.dirName, language);
              if (videoData) {
                return (
                  <iframe 
